library(Mfuzz)
pcl_df = read.csv("/Users/SLancaster/Desktop/Projects/Fiber/Microbiome/Data/Meta_genus_trimmed_NMDS_metadata.txt", sep="\t", header=TRUE, row.names = 1)
bugs_list_df = read.csv("/Users/SLancaster/Desktop/Projects/Fiber/Microbiome/Data/bugs_list.pcl", sep="\t", header=TRUE, row.names = 1)
...
for (i in (pcl_metadata_rows+1):ncol(pcl_df2)) { 
 ...
 to_bind <- tryCatch({ 
    model <- glmer(as.numeric(pcl_df2[,i]) ~ 1 + (1|Dose) + (1|Participant), data=pcl_df2)
 ...
 print(colnames(pcl_df2[i]))
 df <- rbind(df,to_bind)
} 
#
write.table(df, file = paste("/Users/SLancaster/Desktop/pclVariance",fiber_subset,"WeekPartHematoTri.txt",sep=""), sep="\t")
...
df <- data.frame(df)
library(ggtern)
pdf(paste("/Users/SLancaster/Desktop/pclTernary",fiber_subset,"WeekPartHematoTri.pdf",sep=""), width=14)
