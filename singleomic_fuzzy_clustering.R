...
load(paste("/home/slancast/NormAggreg_Log_",fiber_subset,"_pcl_df.RData",sep=""), envir = parent.frame())
...
combined_df <- rbind(t(logaggregnorm_cytokine), t(logaggregnorm_pcl), t(logaggregnorm_rna), t(logaggregnorm_metabolomics), t(logaggregnorm_lipids))
...
if (FALSE) {
print("rna")
df <- t(logaggregnorm_rna)
class(df) <- "numeric"
library(matrixStats)
df <- df[rowVars(df)>0,]
set.seed(1)
library(Mfuzz)
eset <- ExpressionSet(df) #Creating the type expression set with the metadata rows as a different argument
m <- exprs(eset)
eset <- standardise(eset) #Running standarise 
m <- exprs(eset)
m1 = mestimate(eset)
...
for (z in 2:16) {
  ...
  david<-DAVIDWebService(email="slancast@stanford.edu", url="https://david.ncifcrf.gov/webservice/services/DAVIDWebService.DAVIDWebServiceHttpSoap12Endpoint/")
  setTimeOut(david, 100000)
  background <- addList(david, cluster_membership$Gene, idType="ENSEMBL_GENE_ID",listName="Total_genes", listType="Background")
...
for (z in 2:16) {
  ...
  if (FALSE) {
  print("metabolome")
  df <- t(logaggregnorm_metabolomics)
  class(df) <- "numeric"
  library(matrixStats)
  df <- df[rowVars(df)>0,]
  set.seed(1)
...
################################################################################################################
################################################End Metabolome Start Lipidome ##################################
################################################################################################################



  print("lipids")
  df <- t(logaggregnorm_lipids)
  class(df) <- "numeric"
  library(matrixStats)
  df <- df[rowVars(df)>0,]
  set.seed(1)
  ...
  for (z in 2:16) {
    ...
    pdf(paste("/home/slancast/lipids/clusters",z,"multiomics_",fiber_subset,"_parcoord.pdf",sep=""))
    par(mfrow=c(5,4),oma = c(5,4,1,0) + 0.1,
        mar = c(2,1,1,1) + 0.1)
    mfuzz.plot2(eset,cl=mfuzzcl,mfrow=c(4,4), bg="black",ax.col="red",col.lab="black",col.main="green",col.sub="blue",col="blue", Xwidth=20, Xheight=20,colo="fancy", lwd=1,ylab='',xlab='',x11=FALSE,cex.main=1.1,cex.lab=0.1)
    dev.off()
    
    pdf(paste("/home/slancast/lipids/clusters",z,"multiomics_",fiber_subset,"_parcoord2.pdf",sep=""))
    par(mfrow=c(5,4),oma = c(5,4,1,0) + 0.1,
        mar = c(2,1,1,1) + 0.1)
    mfuzz.plot2(eset,cl=mfuzzcl,mfrow=c(4,4), bg="white",ax.col="red",col.lab="black",col.main="green",col.sub="blue",col="blue", Xwidth=20, Xheight=20,colo="fancy", lwd=1,ylab='',xlab='',x11=FALSE,cex.main=1.1,cex.lab=0.1)
    dev.off()
    ...
    write.table(cluster_membership, file = paste("/home/slancast/lipids/clusters",z,"metabolomics_",fiber_subset,"_cluster_membership.txt",sep=""), sep="\t")
  }
