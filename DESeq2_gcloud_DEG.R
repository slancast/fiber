...
samplesa <- samplesa[which(samplesa$week %in% c("Baseline","10","20","30","WashoutD3","WashoutD10")),] #getting rid of the weeks that aren't a part of our expiriment.
samplesa <- samplesa[which(samplesa$fiber %in% c("LCInulin","Arabinoxylan")),] #getting rid of the weeks that aren't a part of our expiriment.


files <- c(files1_2,files3, files4)
names(files) <- paste0("sample", 1:length(files))
files <- files[which(names(files) %in% rownames(samplesa))]
...
j <- "Baseline"
    print(j)
    ddsDESeq <- DESeq(dds, test="LRT", reduced = ~condition + week, parallel = TRUE, BPPARAM=MulticoreParam(15))
    saveRDS(ddsDESeq,file="/home/slancast/RNA/ddstimeseries.rds")
    res <- results(ddsDESeq)
      ...
      #png(paste("~/RNA/rsem-results/deseq_results/DEG_resLFC-maplot.png",sep=""), 1500, 1000, pointsize=20)
      ...
      ## Write results
      write.csv(resdata, file=paste("~/RNA/rsem-results/deseq_results/DEG_-diffexpr-resultsb.csv",sep=""))
      
      ###Visualization and data collection
      
      # Plot dispersions
      png(paste("~/RNA/rsem-results/deseq_results/DEG_qc-dispersions.png",sep=""), 1000, 1000, pointsize=20)
      ...
      #png(paste("~/RNA/rsem-results/deseq_results/DEG_qc-heatmap-samples.png",sep=""), w=1000, h=1000, pointsize=20)
      ...
      #png(paste("~/RNA/rsem-results/deseq_results/DEG_qc-pca.png",sep=""), 1000, 1000, pointsize=20)
      ...
      png(paste("~/RNA/rsem-results/deseq_results/DEG_res-pvalue.png",sep=""), 1000, 1000, pointsize=20)
      hist(res$pvalue, breaks=250, col="grey")
      dev.off()
      
      ## MA plot
      png(paste("~/RNA/rsem-results/deseq_results/DEG_diffexpr-maplot-noLFC.png",sep=""), 1500, 1000, pointsize=20)
      maplot(resdata, ylim = c(-1,1), main="MA Plot")
      dev.off()
      
      ## Volcano plot with "significant" genes labeled
      png(paste("~/RNA/rsem-results/deseq_results/DEG_diffexpr-volcanoplot.png",sep=""), 1200, 1000, pointsize=20)
