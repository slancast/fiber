...
dds <- DESeqDataSetFromTximport(txi, sampleTable, ~ condition + week + condition:week)
...
j <- "Baseline"
    print(j)
    ddsDESeq <- DESeq(dds, test="LRT", reduced = ~strain + minute, parallel = TRUE, BPPARAM=MulticoreParam(15))
    saveRDS(ddsDESeq,file="/home/slancast/RNA/dds.rds")
