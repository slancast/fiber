...
####### RNA-iPOP1_2 #######
####locate the directory containing the files####pri
dir <- "~/rnaseq-storage/rsem-results/rsem_genes_results_all_trimmed"
...
bpparam <- SnowParam(20, log = TRUE, stop.on.error = FALSE)
...
j <- "Baseline"
    ...
    for (h in unique(sampleTable[[i]])){
      ...
      png(paste("~/rnaseq-storage/rsem-results/deseq_results/",j,h,"diffexpr-maplot-preFDRtools.png",sep=""), 1500, 1000, pointsize=20)
      ...
      # resdata <- merge(as.data.frame(resLFC), as.data.frame(counts(ddsDESeq, normalized=TRUE)), by="row.names", sort=FALSE)
      # names(resdata)[1] <- "Gene"
      # 
      # png(paste("~/rnaseq-storage/rsem-results/deseq_results/",j,h,"diffexpr-maplot-LFC.png",sep=""), 1500, 1000, pointsize=20)
      # maplot(resdata, ylim = c(-1,1), main="MA Plot")
      # dev.off()
