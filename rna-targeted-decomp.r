...
for (fiber_subset in c("Arabinoxylan", "LCInulin", "Mix")) { #
  ...
  log_rna <- as.matrix(t(log_rna))
  class(log_rna) <- "numeric"
  log_rna <- log_rna[rowVars(log_rna) > 0,]
  
  log_rna <- data.frame(cbind(t(rna_metadata),t(log_rna)))
  ...
  #log_rna <- log_rna[!log_rna$Dose %in% "WashoutFinal",]
  ...
  mkdir(paste("~/rna",fiber_subset,"/analyte-plots",sep=""))
  
  total_variance <- c()
  eGFR <- gsub("[^0-9\\.]", "", log_rna$eGFR) #removing any non-numeric characters from the data. 
  eGFR <- as.numeric(as.matrix(eGFR))
  eGFR <- eGFR
  eGFR_model <- round(eGFR, digits=0)
  
  Hematocrit <- gsub("[^0-9\\.]", "", log_rna$Hematocrit) #removing any non-numeric characters from the data. 
  Hematocrit <- as.numeric(as.matrix(Hematocrit))
  Hematocrit <- Hematocrit * 10
  Hematocrit_model <- round(Hematocrit, digits=0)
  
  High.Sensitivity.CRP <- gsub("[^0-9\\.]", "", log_rna$High.Sensitivity.CRP) #removing any non-numeric characters from the data. 
  ...
  LDL.HDL.Ratio <- gsub("[^0-9\\.]", "", log_rna$LDL.HDL.Ratio) #removing any non-numeric characters from the data. 
  ...
  Hemoglobin.A1c <- gsub("[^0-9\\.]", "", log_rna$Hemoglobin.A1c) #removing any non-numeric characters from the data. 
  ...
  RBC <- gsub("[^0-9\\.]", "", log_rna$RBC) #removing any non-numeric characters from the data. 
  RBC <- as.numeric(as.matrix(RBC))
  RBC <- RBC * 10
  RBC_model <- round(RBC, digits=0)
  
  Triglyceride..Ser.Plas <- gsub("[^0-9\\.]", "", log_rna$Triglyceride..Ser.Plas) #removing any non-numeric characters from the data. 
  ...
  for (i in (nrow(rna_metadata)+2):ncol(log_rna)) { 
    #print(shapiro.test(as.numeric(log_rna[,i])))
    print(colnames(log_rna)[i])
    response <- as.numeric(as.matrix(log_rna[,i]))
    response <- response * 1000000
    response <- round(response, digits=0)
    if (sum(response) <= 30) next 
    return_list <- tryCatch({ 
      model_nofiber <- lmer(response ~ 1 + (1|participant) + (1|batch) + eGFR_model + Hematocrit_model + High.Sensitivity.CRP_model + LDL.HDL.Ratio_model + RBC_model + Hemoglobin.A1c_model + Triglyceride..Ser.Plas_model, data=log_rna) #, family=poisson(link=identity) Poisson seems to work best at least for metagenome as the response variable
      model <- lmer(response ~ 1 + (1|week) + (1|participant) + (1|batch) + eGFR_model + Hematocrit_model + High.Sensitivity.CRP_model + LDL.HDL.Ratio_model + RBC_model + Hemoglobin.A1c_model + Triglyceride..Ser.Plas_model, data=log_rna) #, family=poisson
      ...
      if (is.na(summary(pbmmG2)$`P(>=obs)`)) {
        stats <- c(pval,sum(variance$vcov),colnames(log_rna[i]),summary(pbmmG2)$`P(>=obs)`,summary(pbmmG2)$`#(>=obs)+noEst/runs`)
      } else {stats <- c(pval,sum(variance$vcov),colnames(log_rna[i]),summary(pbmmG2)$`P(>=obs)`,"NA")
      }
  ...
  write.table(df, file = paste("~/rna",fiber_subset,"/numericCovar",fiber_subset,"WeekPart.txt",sep=""), sep="\t")
  write.table(total_stats, file = paste("~/rna",fiber_subset,"/numericStats",fiber_subset,"WeekPart.txt",sep=""), sep="\t")
  ...
  pdf(paste("~/rna",fiber_subset,"/numericHist",fiber_subset,"eGFRWeekPart.pdf",sep=""))
  hist(as.numeric(as.matrix(total_stats$eGFR), main="eGFR"), breaks=40)
  dev.off()
  
  pdf(paste("~/rna",fiber_subset,"/numericHist",fiber_subset,"HematocritWeekPart.pdf",sep=""))
  hist(as.numeric(as.matrix(total_stats$Hematocrit), main="Hematocrit", xlab="p-value"), breaks=40)
  dev.off()
  
  pdf(paste("~/rna",fiber_subset,"/numericHist",fiber_subset,"High.Sensitivity.CRP_modelWeekPart.pdf",sep=""))
  hist(as.numeric(as.matrix(total_stats$High.Sensitivity.CRP), main="CRP", xlab="p-value"), breaks=40)
  dev.off()
  
  pdf(paste("~/rna",fiber_subset,"/numericHist",fiber_subset,"LDL.HDL.Ratio_model.pdf",sep=""))
  hist(as.numeric(as.matrix(total_stats$LDL.HDL.Ratio), main="LDL", xlab="p-value"), breaks=40)
  dev.off()
  
  pdf(paste("~/rna",fiber_subset,"/numericHist",fiber_subset,"Hemoglobin.A1c_modelWeekPart.pdf",sep=""))
  hist(as.numeric(as.matrix(total_stats$Hemoglobin.A1c), main="A1c", xlab="p-value"), breaks=40)
  dev.off()
  
  pdf(paste("~/rna",fiber_subset,"/numericHist",fiber_subset,"Triglyceride..Ser.Plas_modelWeekPart.pdf",sep=""))
  hist(as.numeric(as.matrix(total_stats$Triglyceride..Ser.Plas), main="Triglyceride", xlab="p-value"), breaks=40)
  dev.off()
  
  pdf(paste("~/rna",fiber_subset,"/numericHist",fiber_subset,"dose-p_WeekPart.pdf",sep=""))
