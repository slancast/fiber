...
dir <- "/srv/gsfs0/projects/snyder/slancast/fiber/rnaseq/fall2019/rsem-results-unaltered/iPOP-3"
...
####### RNA-iPOP4 #######
####locate the directory containing the files####pri
dir <- "/srv/gsfs0/projects/snyder/slancast/fiber/rnaseq/fall2019/rsem-results-unaltered/iPOP-4"
...
####### RNA-iPOP1_2 #######
####locate the directory containing the files####pri
dir <- "/srv/gsfs0/projects/snyder/slancast/fiber/rnaseq/fall2019/rsem-results/rsem_genes_results_all_trimmed"

###create vector of filenames from table
samples <- read.table(file.path(dir, "samples.txt"), header = TRUE)
samples <- samples[samplesa$fiber=="Arabinoxylan",]
...
###
#samplesa <- rbind(samplesa, samples3a, samples4a)
rownames(samplesa) <- paste0("sample", 1:nrow(samplesa))

#For testing on Arabinoxylan
...
bpparam <- SnowParam(5, log = TRUE, stop.on.error = FALSE)
...
sampleTable$participant <- samplesa$participant
...
dds <- DESeqDataSetFromTximport(txi, sampleTable, ~week)
###prefilter low count genes

dds <- dds[ rowMeans(counts(dds)) > 0.8, ]
...
ddsDESeq <- DESeq(ddsDESeq, parallel = TRUE, BPPARAM=MulticoreParam(5))
saveRDS(ddsDESeq,file="/srv/gsfs0/projects/snyder/slancast/fiber/rnaseq/fall2019/dds_baseline_participant.rds")
...
for (h in unique(sampleTable[[i]])){
  if (h == j) next
  print(h)
  #, cooksCutoff=FALSE, independentFiltering=FALSE, altHypothesis = "greaterAbs"
  res <- results(ddsDESeq,  contrast = c("week","Baseline","30"), parallel = TRUE, BPPARAM=MulticoreParam(5))
  
  
  print("LFCShrink")
  #resLFC <- lfcShrink(ddsDESeq, res=res,coef=paste("fiber_week_",h,"_vs_",j,sep=""), type="apeglm",parallel = TRUE, BPPARAM=MulticoreParam(5))
  print("LFCShrinkDone")
  

  #res <- na.omit(res)
  print("res created")
  res.fixed <- fdrtool(res$stat, plot=FALSE, statistic = "normal") #adjusting for batch effects (https://support.bioconductor.org/p/99685/; http://www-huber.embl.de/users/klaus/Teaching/DESeq2Predoc2014.html)
  ...
  #png(paste("/srv/gsfs0/projects/snyder/slancast/fiber/rnaseq/fall2019/rsem-results/deseq_results/",h,j,"resLFC-maplot.png",sep=""), 1500, 1000, pointsize=20)
  ...
  ## Write results
  write.csv(resdata, file=paste("/srv/gsfs0/projects/snyder/slancast/fiber/rnaseq/fall2019/rsem-results/deseq_results/",h,j,"-diffexpr-resultsb.csv",sep=""))
  
  ###Visualization and data collection
  
  # Plot dispersions
  png(paste("/srv/gsfs0/projects/snyder/slancast/fiber/rnaseq/fall2019/rsem-results/deseq_results/",h,j,"qc-dispersions.png",sep=""), 1000, 1000, pointsize=20)
  ...
  #rld <- bplapply(ddsDESeq, rlogTransformation(ddsDESeq), BPPARAM=MulticoreParam(5)) 
  #head(assay(rld))
  #hist(assay(rld))
  ...
  #png(paste("/srv/gsfs0/projects/snyder/slancast/fiber/rnaseq/fall2019/rsem-results/deseq_results/",h,j,"qc-heatmap-samples.png",sep=""), w=1000, h=1000, pointsize=20)
  ...
  #png(paste("/srv/gsfs0/projects/snyder/slancast/fiber/rnaseq/fall2019/rsem-results/deseq_results/",h,j,"qc-pca.png",sep=""), 1000, 1000, pointsize=20)
  ...
  png(paste("/srv/gsfs0/projects/snyder/slancast/fiber/rnaseq/fall2019/rsem-results/deseq_results/",h,j,"res-pvalue.png",sep=""), 1000, 1000, pointsize=20)
  hist(res$pvalue, breaks=250, col="grey")
  dev.off()
  
  ## MA plot
  png(paste("/srv/gsfs0/projects/snyder/slancast/fiber/rnaseq/fall2019/rsem-results/deseq_results/",h,j,"diffexpr-maplot-noLFC.png",sep=""), 1500, 1000, pointsize=20)
  maplot(resdata, ylim = c(-1,1), main="MA Plot")
  dev.off()
  
  ## Volcano plot with "significant" genes labeled
  png(paste("/srv/gsfs0/projects/snyder/slancast/fiber/rnaseq/fall2019/rsem-results/deseq_results/",h,j,"diffexpr-volcanoplot.png",sep=""), 1200, 1000, pointsize=20)
