...
####### RNA-iPOP1_2 #######
####locate the directory containing the files####pri
dir <- "~/rsem-results/rsem_genes_results_all_trimmed"
...
samplesa <- samples[,-1]
samples4a <- cbind(samples4[,7],samples4[,8:9],samples4[,6])
colnames(samples4a) <- colnames(samplesa)
samples3a <- cbind(samples3[,7],samples3[,8:9],samples3[,6])
...
files <- c(files1_2,files3, files4)
...
dds <- DESeqDataSetFromTximport(txi, sampleTable, ~participant + fiber_week)
...
j <- "Baseline"
    ...
    saveRDS(ddsDESeq,file="/home/slancast/RNA/dds_baseline_participant.rds")
    ...
    for (h in unique(sampleTable[[i]])){
      ...
      #png(paste("~/RNA/rsem-results/deseq_results/",j,h,"resLFC-maplot.png",sep=""), 1500, 1000, pointsize=20)
      ...
      ## Write results
      write.csv(resdata, file=paste("~/RNA/rsem-results/deseq_results/",j,h,"-diffexpr-resultsb.csv",sep=""))
      
      ###Visualization and data collection
      
      # Plot dispersions
      png(paste("~/RNA/rsem-results/deseq_results/",j,h,"qc-dispersions.png",sep=""), 1000, 1000, pointsize=20)
      ...
      #png(paste("~/RNA/rsem-results/deseq_results/",j,h,"qc-heatmap-samples.png",sep=""), w=1000, h=1000, pointsize=20)
      ...
      #png(paste("~/RNA/rsem-results/deseq_results/",j,h,"qc-pca.png",sep=""), 1000, 1000, pointsize=20)
      ...
      png(paste("~/RNA/rsem-results/deseq_results/",j,h,"res-pvalue.png",sep=""), 1000, 1000, pointsize=20)
      hist(res$pvalue, breaks=250, col="grey")
      dev.off()
      
      ## MA plot
      png(paste("~/RNA/rsem-results/deseq_results/",j,h,"diffexpr-maplot-noLFC.png",sep=""), 1500, 1000, pointsize=20)
      maplot(resdata, ylim = c(-1,1), main="MA Plot")
      dev.off()
      
      ## Volcano plot with "significant" genes labeled
      png(paste("~/RNA/rsem-results/deseq_results/",j,h,"diffexpr-volcanoplot.png",sep=""), 1200, 1000, pointsize=20)
...
#####################################################
    ...
    if (FALSE) {
      write.csv(filtering_table, file=paste("~/RNA/rsem-results/deseq_results/",j,h,"filtering_table.csv",sep=""))
      png(paste("~/RNA/rsem-results/deseq_results/",j,h,"cooks_boxplot_1-50.png",sep=""), 1000, 1000, pointsize=20)
      boxplot(log10(assays(ddsDESeq)[["cooks"]][,1:50]), range=0, las=2)
      dev.off()
      png(paste("~/RNA/rsem-results/deseq_results/",j,h,"cooks_boxplot_51-100.png",sep=""), 1000, 1000, pointsize=20)
      boxplot(log10(assays(ddsDESeq)[["cooks"]][,51:100]), range=0, las=2)
      dev.off()
      png(paste("~/RNA/rsem-results/deseq_results/",j,h,"cooks_boxplot_101-150.png",sep=""), 1000, 1000, pointsize=20)
      boxplot(log10(assays(ddsDESeq)[["cooks"]][,101:150]), range=0, las=2)
      dev.off()
      png(paste("~/RNA/rsem-results/deseq_results/",j,h,"cooks_boxplot_151-200.png",sep=""), 1000, 1000, pointsize=20)
      boxplot(log10(assays(ddsDESeq)[["cooks"]][,151:200]), range=0, las=2)
      dev.off()
      png(paste("~/RNA/rsem-results/deseq_results/",j,h,"cooks_boxplot_201-250.png",sep=""), 1000, 1000, pointsize=20)
      boxplot(log10(assays(ddsDESeq)[["cooks"]][,201:250]), range=0, las=2)
      dev.off()
      png(paste("~/RNA/rsem-results/deseq_results/",j,h,"cooks_boxplot_251-301.png",sep=""), 1000, 1000, pointsize=20)
