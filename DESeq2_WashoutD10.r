...
#This program will be to run DESeq2 on the WashoutD10 samples, which in explicably didn't work on the cluster
#I troubleshooted it (see recap, 10/5/17) but to no avail. It seemed to work on my desktop,
#so I'm going to proceed with that.
...
#dir <- "/srv/gsfs0/projects/snyder/slancast/fiber/rnaseq/Projects/Fiber/Data/rsem_genes_results_all_trimmed"
...
sampleTable <- data.frame(week = factor(samples$week))
...
dds <- dds[ rowMeans(counts(dds)) > 0.2, ]
  ...
  ddsDESeq <- dds
  for (j in unique(sampleTable$week)) { 
    print(j)
    #change to/from j
    ddsDESeq$week <- relevel(ddsDESeq$week, ref = "WashoutD10")   
    ###Differential analysis
    
    ddsDESeq <- DESeq(ddsDESeq)
    
    resultsNames(ddsDESeq)
    print(resultsNames(ddsDESeq)[2:length(resultsNames(ddsDESeq))])
...
#      res$padj <- padjusted
      ...
      print(paste("/srv/gsfs0/projects/snyder/slancast/fiber/rnaseq/RNAseq_results/",h,"resLFC-maplot.png",sep=""))
