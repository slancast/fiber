...
rna_df <- data.frame(t(rna_df))
  ...
  for (i in (rna_metadata_rows+1):ncol(rna_df)) {
    ...
    to_bind <- tryCatch({
      model <- glmer(as.numeric(as.character(rna_df[,i])) ~ 1 + (1|week) + (1|participant) + as.numeric(as.character(rna_df$Hemoglobin.A1c)) + as.numeric(as.character(rna_df$LYM...)), data=rna_df)
  ...
  #
  write.table(df, file = paste("./RNA/WeekPart.txt",sep=""), sep="\t")
  write.table(total_variance, file = paste("./RNA/totvarRNAAxWeekPartA1cLym",fiber_subset,".txt",sep=""), sep="\t")
  ...
  pdf(paste("./RNA/WeekPartA1cDateLym",fiber_subset,".pdf",sep=""), width=14)
