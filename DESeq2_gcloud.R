...
# for (i in files3){
#   print(i)
#   rsem_file <- read.table(i, sep="\t", header=TRUE)
#   rsem_file <-data.frame(rsem_file)
#   rsem_file <- rsem_file[grepl("^E", rsem_file[,1]),]
#   rsem_file <- rsem_file[!grepl("^ENSGR", rsem_file[,1]),]
#   write.table(rsem_file,file=i,sep="\t",quote=FALSE)
# }
...
dir <- "~/rsem-results/iPOP-1-2"
setwd(dir)
#unlink("*SC_Inulin*")
#file.rename(from=list.files(dir),to=gsub("LC_Inulin","LCInulin",list.files(dir)))
#file.rename(from=list.files(dir),to=gsub("Washout_D","WashoutD",list.files(dir)))
...
#   rsem_file <- read.table(i, sep="\t", header=TRUE)
#   rsem_file <-data.frame(rsem_file)
#   to_remove <- c("TPM_coefficient_of_quartile_variation", "FPKM_coefficient_of_quartile_variation")
#   rsem_file <- rsem_file[,which(!colnames(rsem_file) %in% to_remove)]
#   colnames(rsem_file)[2] <- "transcript_id(s)"
#   write.table(rsem_file,file=i,sep="\t",quote=FALSE)
# }
}
...
#What going on here is I've made all the samples placed them in order and then generated the list of files from
#those samples and listed them 1:end of samples. That way file 1 will match sample1
...
sampleTable[,4] <- data.frame(batch = factor(samplesa$batch))
...
i <- colnames(sampleTable)[3]
  ...
  for (j in unique(sampleTable[[i]])) { 
    ...
    print("releveled")
    #betaPrior = TRUE,
    ddsDESeq <- DESeq(ddsDESeq, parallel = TRUE, BPPARAM=MulticoreParam(15)) #The DESeq function must come here because of the releveling
    ...
    for (h in unique(sampleTable[[i]])){
      ...
      print("LFCShrink")
      #resLFC <- lfcShrink(ddsDESeq, coef=2, res=res,parallel = TRUE, BPPARAM=MulticoreParam(30))
      print("LFCShrinkDone")
      #      names(resLFC)[1] <- "Gene"
      ## Examine independent filtering
      #attr(res, "filterThreshold")
      #plot(attr(res,"filterNumRej"), type="b", xlab="quantiles of baseMean", ylab="number of rejections")
      #png(paste("~/rnaseq-storage/rsem-results/deseq_results/",j,h,"res-maplot.png",sep=""), 1500, 1000, pointsize=20)
